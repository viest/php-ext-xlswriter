...
static int begin_font(lxlsx_reader_styles *st)
{
    ...
    memset(&st->fonts[st->fonts_count], 0, sizeof(lxlsx_reader_font));
    st->fonts[st->fonts_count].underline = LXLSX_READER_UNDERLINE_NONE;
    return 0;
}
...
static int begin_fill(lxlsx_reader_styles *st)
{
    ...
    memset(&st->fills[st->fills_count], 0, sizeof(lxlsx_reader_fill));
    return 0;
}
...
static int begin_border(lxlsx_reader_styles *st)
{
    ...
    memset(&st->borders[st->borders_count], 0, sizeof(lxlsx_reader_border));
    return 0;
}
...
static void extract_color(const char **attrs, char out[LXLSX_READER_COLOR_LEN])
{
    out[0] = 0;
    if (!attrs) return;
    const char *rgb = lxlsx_reader_xml_attr(attrs, "rgb");
    if (rgb) {
        size_t n = strlen(rgb);
        if (n >= LXLSX_READER_COLOR_LEN) n = LXLSX_READER_COLOR_LEN - 1;
        memcpy(out, rgb, n);
        out[n] = 0;
    }
...
static void on_start(void *ud, const char *name, const char **attrs)
{
    ...
    if (sc == SC_FONT) {
        ...
        } else if (lxlsx_reader_xml_name_eq(name, "b"))      { f->bold      = parse_bool_attr(attrs); }
        else if   (lxlsx_reader_xml_name_eq(name, "i"))      { f->italic    = parse_bool_attr(attrs); }
        else if   (lxlsx_reader_xml_name_eq(name, "strike")) { f->strike    = parse_bool_attr(attrs); }
    ...
    if (sc == SC_FILL && lxlsx_reader_xml_name_eq(name, "patternFill")) {
        ...
        const char *pt  = lxlsx_reader_xml_attr(attrs, "patternType");
    ...
    if (sc == SC_PATTERN_FILL) {
        lxlsx_reader_fill *fl = &c->st->fills[c->st->fills_count];
        if      (lxlsx_reader_xml_name_eq(name, "fgColor")) extract_color(attrs, fl->fg_color);
        else if (lxlsx_reader_xml_name_eq(name, "bgColor")) extract_color(attrs, fl->bg_color);
        return;
    }
    ...
    if (sc == SC_BORDER) {
        lxlsx_reader_border *b   = &c->st->borders[c->st->borders_count];
        lxlsx_reader_border_side *side = NULL;
        scope_t          ns   = SC_BORDER;

        if      (lxlsx_reader_xml_name_eq(name, "left"))   { side = &b->left;   ns = SC_BORDER_LEFT; }
        else if (lxlsx_reader_xml_name_eq(name, "right"))  { side = &b->right;  ns = SC_BORDER_RIGHT; }
        else if (lxlsx_reader_xml_name_eq(name, "top"))    { side = &b->top;    ns = SC_BORDER_TOP; }
        else if (lxlsx_reader_xml_name_eq(name, "bottom")) { side = &b->bottom; ns = SC_BORDER_BOTTOM; }
