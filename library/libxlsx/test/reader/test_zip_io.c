...
{
    lxlsx_reader_zip *z = lxlsx_reader_zip_open_path("/nonexistent/path/to/file.xlsx");
    TEST_ASSERT_NULL(z);
}

static void test_open_path_returns_null_on_null_arg(void)
{
    TEST_ASSERT_NULL(lxlsx_reader_zip_open_path(NULL));
}
...
static void test_entry_exists_known_files(void)
{
    ...
    TEST_ASSERT_TRUE(lxlsx_reader_zip_entry_exists(z, "[Content_Types].xml"));
    TEST_ASSERT_TRUE(lxlsx_reader_zip_entry_exists(z, "xl/workbook.xml"));
    TEST_ASSERT_TRUE(lxlsx_reader_zip_entry_exists(z, "xl/worksheets/sheet1.xml"));
    TEST_ASSERT_FALSE(lxlsx_reader_zip_entry_exists(z, "xl/worksheets/sheet999.xml"));
    lxlsx_reader_zip_close(z);
}
...
static void test_read_workbook_xml(void)
{
    ...
    n = lxlsx_reader_zip_read(zf, buf, sizeof(buf) - 1);
...
static void test_read_drains_to_eof(void)
{
    ...
    TEST_ASSERT_NOT_NULL(z);
    zf = lxlsx_reader_zip_open_entry(z, "xl/sharedStrings.xml");
    TEST_ASSERT_NOT_NULL(zf);

    while ((n = lxlsx_reader_zip_read(zf, buf, sizeof(buf))) > 0) total += (size_t)n;
...
{
    lxlsx_reader_zip *z = lxlsx_reader_zip_open_path(XLSX);
    lxlsx_reader_zip_file *zf = lxlsx_reader_zip_open_entry(z, "xl/no_such_thing.xml");
    TEST_ASSERT_NULL(zf);
    lxlsx_reader_zip_close(z);
}
...
static void test_iterate_entries(void)
{
    ...
    lxlsx_reader_zip *z = lxlsx_reader_zip_open_path(XLSX);
    TEST_ASSERT_NOT_NULL(z);
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_zip_iterate_entries(z, collect_iter, &st));
...
static void test_open_memory(void)
{
    ...
    z = lxlsx_reader_zip_open_memory(data, len);
    TEST_ASSERT_NOT_NULL(z);
    TEST_ASSERT_TRUE(lxlsx_reader_zip_entry_exists(z, "xl/workbook.xml"));
    lxlsx_reader_zip_close(z);
    free(data);
