...
static void on_start(void *ud, const char *name, const char **attrs)
{
    ...
    if (lxlsx_reader_xml_name_eq(name, "root")) s->saw_root = 1;
    if (lxlsx_reader_xml_name_eq(name, "child")) s->saw_namespaced = 1;
    (void)attrs;
}
...
static void test_parse_simple_buffer(void)
{
    ...
    counter_state st = {0};
    lxlsx_reader_xml_pump *p = lxlsx_reader_xml_pump_create_buffer(xml, strlen(xml));
    TEST_ASSERT_NOT_NULL(p);
    lxlsx_reader_xml_pump_set_handlers(p, on_start, on_end, on_text, &st);
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_xml_pump_run(p));
    TEST_ASSERT_TRUE(lxlsx_reader_xml_pump_is_eof(p));
...
static void test_parse_error_returns_xml_parse(void)
{
    ...
    TEST_ASSERT_NOT_NULL(p);
    lxlsx_reader_xml_pump_set_handlers(p, on_start, on_end, on_text, &st);
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_ERROR_XML_PARSE, lxlsx_reader_xml_pump_run(p));
    lxlsx_reader_xml_pump_destroy(p);
}
...
static void capture_start(void *ud, const char *name, const char **attrs)
{
    ...
    if (lxlsx_reader_xml_name_eq(name, "cell")) {
        const char *v = lxlsx_reader_xml_attr(attrs, s->target_attr);
        if (v) {
...
static void test_attr_lookup(void)
{
    ...
    p = lxlsx_reader_xml_pump_create_buffer(xml, strlen(xml));
    TEST_ASSERT_NOT_NULL(p);
    lxlsx_reader_xml_pump_set_handlers(p, capture_start, NULL, NULL, &st);
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_xml_pump_run(p));
...
static void test_suspend_and_resume(void)
{
    ...
    TEST_ASSERT_NOT_NULL(p);
    st.pump = p;
    lxlsx_reader_xml_pump_set_handlers(p, suspend_start, NULL, NULL, &st);

    /* Initial run: suspends after the first <row> */
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_xml_pump_run(p));
    TEST_ASSERT_TRUE(lxlsx_reader_xml_pump_is_suspended(p));
    TEST_ASSERT_EQUAL_INT(1, st.row_starts);

    while (lxlsx_reader_xml_pump_is_suspended(p) && !lxlsx_reader_xml_pump_is_eof(p)) {
        TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_xml_pump_resume(p));
...
static void test_pump_on_zip_file(void)
{
    ...
    zf = lxlsx_reader_zip_open_entry(z, "xl/workbook.xml");
    TEST_ASSERT_NOT_NULL(zf);

    p = lxlsx_reader_xml_pump_create_zip_file(zf);
    TEST_ASSERT_NOT_NULL(p);
    lxlsx_reader_xml_pump_set_handlers(p, on_start, on_end, on_text, &st);
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_xml_pump_run(p));
    TEST_ASSERT_TRUE(lxlsx_reader_xml_pump_is_eof(p));
...
{
    TEST_ASSERT_TRUE (lxlsx_reader_xml_name_eq("c", "c"));
    TEST_ASSERT_TRUE (lxlsx_reader_xml_name_eq("x:c", "c"));
    TEST_ASSERT_FALSE(lxlsx_reader_xml_name_eq("xc", "c"));
    TEST_ASSERT_FALSE(lxlsx_reader_xml_name_eq("c", "x:c"));
    TEST_ASSERT_FALSE(lxlsx_reader_xml_name_eq(NULL, "c"));
    TEST_ASSERT_FALSE(lxlsx_reader_xml_name_eq("c", NULL));
}
