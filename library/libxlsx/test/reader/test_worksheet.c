...
static int cell_cb(const lxlsx_reader_cell *c, void *ud)
{
...
static int type_cell_cb(const lxlsx_reader_cell *c, void *ud)
{
    type_state *s = (type_state *)ud;
    s->cells_seen++;
    switch (c->type) {
    case LXLSX_READER_CELL_NUMBER:   s->n_number++;   break;
    case LXLSX_READER_CELL_STRING:
        s->n_string++;
        if (c->row == 2 && c->col == 1) {
        ...
        break;
    case LXLSX_READER_CELL_BOOLEAN:
        s->n_boolean++;
        if (c->value.boolean) s->bool_true_seen = 1;
        else                  s->bool_false_seen = 1;
        break;
    case LXLSX_READER_CELL_ERROR:
        s->n_error++;
        strncpy(s->err_code, c->value.error_code, sizeof(s->err_code) - 1);
        break;
    case LXLSX_READER_CELL_FORMULA:   s->n_formula++;  break;
    case LXLSX_READER_CELL_INLINE_STRING: s->n_inline++; break;
    case LXLSX_READER_CELL_DATETIME:  s->n_datetime++; break;
    default: break;
    }
    return 0;
}
...
static void test_pull_pulls_correct_values(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_NO_ERROR, lxlsx_reader_worksheet_next_cell(ws, &c));
    TEST_ASSERT_EQUAL_INT(LXLSX_READER_CELL_NUMBER, c.type);
