...
#define LXLSX_READER_PARSE_BUFFER_SIZE (64 * 1024)

typedef enum {
    LXLSX_READER_SOURCE_CALLBACK,
    LXLSX_READER_SOURCE_ZIP_FILE,
    LXLSX_READER_SOURCE_BUFFER
} lxlsx_reader_source_kind;

struct lxlsx_reader_xml_pump {
    XML_Parser parser;

    lxlsx_reader_source_kind source;
    lxlsx_reader_xml_read_fn read_fn;
    void           *read_ud;
...
{
    return lxlsx_reader_zip_read((lxlsx_reader_zip_file *)ud, buf, n);
}

static ssize_t read_from_buffer(void *ud, void *buf, size_t n)
{
    lxlsx_reader_xml_pump *p = (lxlsx_reader_xml_pump *)ud;
...
static lxlsx_reader_xml_pump *new_pump(void)
{
    lxlsx_reader_xml_pump *p = (lxlsx_reader_xml_pump *)calloc(1, sizeof(*p));
    if (!p) return NULL;
...
lxlsx_reader_xml_pump *lxlsx_reader_xml_pump_create_callback(lxlsx_reader_xml_read_fn fn, void *ud)
{
    lxlsx_reader_xml_pump *p;
    if (!fn) return NULL;
    p = new_pump();
    if (!p) return NULL;
    p->source  = LXLSX_READER_SOURCE_CALLBACK;
    p->read_fn = fn;
    p->read_ud = ud;
    return p;
}
...
lxlsx_reader_xml_pump *lxlsx_reader_xml_pump_create_zip_file(lxlsx_reader_zip_file *zf)
{
    ...
    p = new_pump();
    if (!p) return NULL;
    p->source  = LXLSX_READER_SOURCE_ZIP_FILE;
...
lxlsx_reader_xml_pump *lxlsx_reader_xml_pump_create_buffer(const char *data, size_t len)
{
    ...
    p = new_pump();
    if (!p) return NULL;
    p->source   = LXLSX_READER_SOURCE_BUFFER;
    p->buf_data = data;
    p->buf_len  = len;
    p->buf_pos  = 0;
...
typedef struct {
    ...
    lxlsx_reader_xml_start_fn start;
    lxlsx_reader_xml_end_fn   end;
    lxlsx_reader_xml_text_fn  text;
    void            *ud;
} lxlsx_reader_xml_bridge;

static void XMLCALL bridge_start(void *ud, const XML_Char *name, const XML_Char **attrs)
{
    lxlsx_reader_xml_bridge *b = (lxlsx_reader_xml_bridge *)ud;
                               ...
                               void            *ud)
{
    ...
    b = (lxlsx_reader_xml_bridge *)XML_GetUserData(p->parser);
    if (!b) {
        b = (lxlsx_reader_xml_bridge *)calloc(1, sizeof(*b));
        if (!b) return;
        b->pump = p;
...
static lxlsx_reader_error pump_loop(lxlsx_reader_xml_pump *p, int starting_fresh)
{
    enum XML_Status status;

    if (!p) return LXLSX_READER_ERROR_NULL_PARAMETER;

    if (p->error) return LXLSX_READER_ERROR_XML_PARSE;
    ...
    if (!starting_fresh && p->suspended) {
        ...
        if (status == XML_STATUS_ERROR) {
            ...
            if (XML_GetErrorCode(p->parser) != XML_ERROR_NOT_SUSPENDED) {
                p->error = 1;
                return LXLSX_READER_ERROR_XML_PARSE;
            }
        }
    }
    ...
    while (!p->eof && !p->suspended) {
        ssize_t got;
        void *buf = XML_GetBuffer(p->parser, LXLSX_READER_PARSE_BUFFER_SIZE);
        if (!buf) {
            p->error = 1;
            return LXLSX_READER_ERROR_MEMORY_MALLOC_FAILED;
        }

        got = p->read_fn(p->read_ud, buf, LXLSX_READER_PARSE_BUFFER_SIZE);
        if (got < 0) {
            p->error = 1;
...
const char *lxlsx_reader_xml_attr(const char **attrs, const char *name)
{
    ...
    while (*attrs) {
        if (lxlsx_reader_xml_name_eq(*attrs, name)) return *(attrs + 1);
        attrs += 2;
    }
    return NULL;
}
