...
lxlsx_workbook_free(lxlsx_workbook *workbook)
{
    ...
    if (workbook->edit_session) {
        lxlsx_edit_close(workbook->edit_session);
        workbook->edit_session = NULL;
    }
...
lxlsx_workbook *
lxlsx_workbook_open(const char *filename)
{
    lxlsx_workbook *workbook;
    ...
    session = lxlsx_edit_open(filename);
    if (!session)
        return NULL;

    workbook = lxlsx_workbook_new(filename);
    if (!workbook) {
    ...
    for (i = 0; i < sheet_count; i++) {
        ...
        worksheet = lxlsx_workbook_add_worksheet(workbook, sheet_name);
        if (!worksheet) {
            lxlsx_workbook_free(workbook);
            return NULL;
        }

        worksheet->is_edit = LXLSX_TRUE;
        worksheet->edit_session = session;
        worksheet->edit_sheet_name = lxlsx_strdup(sheet_name);
        if (!worksheet->edit_sheet_name) {
            lxlsx_workbook_free(workbook);
            return NULL;
        }
    }

    if (sheet_count > 0 && workbook->num_sheets == 0) {
        lxlsx_workbook_free(workbook);
        return NULL;
    }
...
uint8_t
lxlsx_workbook_is_edit(lxlsx_workbook *workbook)
{
    return workbook && workbook->is_edit;
}
